...
echo 1. Regedit (43 otimizações)
echo 2. Desativação de serviços desnecessários (84 serviços)
echo 3. Comandos no cmd (15 comandos)
echo 4. Limpeza de arquivos temporários (Eliminação de arquivos .log no pc e das pastas: recent, prefetch, %%temp%%, temp)
echo.
...
echo Assim melhorando o inputlag e deixando o sistema operacional e os jogos mais estáveis. 

echo.

echo Algumas partes do Windows que podem não funcionar corretamente:
echo Windows Update, Windows Copilot, Cortana, XboxGameBar, Conectar a uma impressora ou imprimir algo pelo computador, Máquinas Virtuais
...
echo 4 Alterações feitas em HKCU\System\GameConfigStore.
...
reg add "HKLM\SYSTEM\CurrentControlSet\Control\PriorityControl" /v "Win32Priority" /t REG_DWORD /d 26 /f >nul 2>&1
...
takeown /f "HKLM\SOFTWARE\Microsoft\WindowsRuntime\ActivatableClassId\Windows.Gaming.GameBar.PresenceServer.Internal.PresenceWriter" /r /d y >nul 2>&1
icacls "HKLM\SOFTWARE\Microsoft\WindowsRuntime\ActivatableClassId\Windows.Gaming.GameBar.PresenceServer.Internal.PresenceWriter" /grant %username%:F >nul 2>&1
...
echo 1 Alteração feita em HKLM\SOFTWARE\Microsoft\WindowsRuntime\ActivatableClassId\Windows.Gaming.GameBar.PresenceServer.Internal.PresenceWriter
echo.
...
echo Para ver serviços presione Win + R e digite services.msc.
...
echo Widgets do Windows desativados.
...
echo Arquivos .log ou .logs foram limpos.
